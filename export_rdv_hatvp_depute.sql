...
WHERE app.CreatedOn >= '2023-01-01'
  AND EXISTS (
        SELECT 1
        FROM (
            SELECT ap.PartyId AS ContactId
            FROM dbo.ActivityParty AS ap
            WHERE ap.ActivityId            = app.ActivityId
              AND ap.ParticipationTypeMask = 5
              AND ap.IsPartyDeleted        = 0
            ...
            WHERE ap.ActivityId            = app.ActivityId
              ...
              AND ap.IsPartyDeleted        = 0
            UNION
            SELECT app.RegardingObjectId
            WHERE app.RegardingObjectTypeCode = 2
        ) AS tous_contacts
        INNER JOIN dbo.Contact AS c
            ON  c.ContactId         = tous_contacts.ContactId
        INNER JOIN dbo.StringMap AS sm_f
            ON  sm_f.ObjectTypeCode = 2
            AND sm_f.AttributeName  = 'grdf_fonction'
            AND sm_f.AttributeValue = c.grdf_fonction
            AND sm_f.LangId         = 1036
            AND sm_f.Value          = 'Député'
  )
GROUP BY
    app.ActivityId,
    app.grdf_hatvp,
